...
cleandat2 <- cleandat1 %>% 
  mutate(species = str_remove(species, " Penguin \\(Pygoscelis adeliae\\)") %>% 
           str_remove(" penguin \\(Pygoscelis papua\\)") %>% 
           str_remove(" penguin \\(Pygoscelis antarctica\\)"))
...
*Note*: Time zones are important in R!!! If not specified, the dates and times may be assigned the time zone of your computer system, which may be misleading if the data was collected somewhere completely different (e.g., penguins in Antarctica!). Some functions (e.g., `read_csv`) will automatically assign the `UTC` time zone, while others may assign the time zone of your computer. It is thus critical to check the time zone of any date and time objects prior to doing any data manipulation. This can be done with the `tz()` function. 
...
# time zone of date variable from the palmerpenguins package
tz(cleandat5$date_egg) 
...
cleandat6 <- cleandat5 %>% 
  mutate(date_time_utc = as_datetime(date_egg) + time_sec,  # Create a date and time variable in UTC
         date_time_utc03 = with_tz(date_time_utc, tzone = "Etc/GMT+3")) # Convert to GMT-03
                                    
# look at the first few entries to ensure that everything was converted correctly
cleandat6 %>% 
  select(date_egg, time_sec, date_time_utc, date_time_utc03) %>% 
  head(n = 10)

# check time zones
tz(cleandat6$date_time_utc)
tz(cleandat6$date_time_utc03)
...
**Exercise:** The maximum flipper length summary contains NA values for sex. Generate this summary without those values.
