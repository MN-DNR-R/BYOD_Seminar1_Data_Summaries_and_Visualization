...
Using R and R scripts ensures that the original dataset is kept unaltered, and that all data manipulation steps are documented and reproducible. This is a major advantage of tidying data in R as opposed to using spreadsheet applications such as Excel, where there is often few to no records of changes made to a dataset.

## Introducing pipe operator

The `tidyr` syntax uses the *pipe* operator `%>%` or `|>`, which provides a way to efficiently chain functions together in a single statement. It is similar to saying: let's take this data, *and then* do this with it, *and then* do this, etc. So ultimately, every time that you see or use `%>%` in a script, you can think of it as being the same as if you were reading or coding *and then...*!
...
The operator `|>` is from base R while `%>%` is from the `tidyverse` package `magrittr`.
