...
Now that we've formatted our date and time variable as POSIXct project with the appropriate time zone, we can extract all sorts of useful information very easily (e.g., hour(), month(), year()).
...
[Rougier et al.](https://doi.org/10.1371/journal.pcbi.1003833) present ten simple rules for better figures in ther 2014 paper:
...
Let's add labels and make the graph look a little bit nice by removing the grey background color. Labels can be added using the `labs()` function, and the theme can be adjusted to change the overall appearance of the figure. Here, we use a black-and-white theme, but there are many available.
