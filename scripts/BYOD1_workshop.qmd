...
Before doing any analyses in R it is important to recognize the various possible data types. When importing your data into an R session, R will automatically recognize the type of data of each column in a given data frame. At times, however, a specific datatype may need to be set, or converted from one to another. For example, one could use the `as.numeric()` function to convert a column to *numeric*, or `as.character()` to convert to *character*. When doing such data conversion, it is important to note that some elements of a vector may be converted to NAs if R recognizes that it does not correspond to the data type specified. </p>

A good example of this is when a column that should be *numeric* contains characters (e.g., instead of an actual counts one had recorded something like *more then 100 animals*). In this case, this would return an *NA* as R does not know what to do with this information, and neither do we! </p>

Good data entry and data quality check practices is thus very important prior to importing any data in an R session or project.
...
# The str function is out of base R and also provides a quick but complete overview of the dataset and data types as detected by R
str(rawdat)
...
The *pipe* operator `%>%` or `|>` provides a way to efficiently chain functions together in a single statement. It is similar to saying: let's take this data, *and then* do this with it, *and then* do this, etc. So ultimately, everything time that you see or use `%>%` in a script, you can think of it as being the same as if you were reading or coding *and then...*!</p>
...
The time variable will be in total number of seconds from 00:00:00. We will learn how to re-format this in proper time stamp (hour:minute:second) so that it is more intuitive to work with and visualize. Times in seconds frequently happen with GPS Location data, for example. It isn't a bad idea to have a way to convert them in your toolbox!</p>
...
We have now generated a new variable `time_sec` which reflects the time of day in seconds from the starting point 00:00:00. We will learn below how to re-format those times so that they are in the appropriate time zone. </p>

*Note*: Time zones are important in R!!! If not specified, the dates and times will be assigned the time zone of your computer system, which may be misleading if the data was collected somewhere completely different (e.g., penguins in Antartica!).</p>
...
Creating data summaries is often the first step of a thorough Exploratory Data Analysis (EDA). The `group_by` and `summarise` functions are some of the most useful functions you'll find in R as they are very intuitive and efficient. </p>

We will summarise the penguin data to illustrate what can be done with these two functions. Let's first take a look at the data again using `glimpse` to refresh our memory on the class of data that we have on hand. </p>
...
Above we created a summary of mean, standard deviation, minimum, and maximum flipper length. There is no limit on the number of functions we can calculate with thing the `summarise` statement. </p>
...
Here is a quick guide to get you started based on the steps we have following with the palmer penguins data, but feel free to go on your own as you see fit! </p>

1. Start a new project and R script.</p>
1. Load libraries (`tidyverse` and others as needed)</p>
1. Read your data in R using the function that correspond to your file type (`read_csv`(),`read_excel`())</p>
1. Take a first look at the data using `glimpse()`,`str()`,`dim()`,`nrow()`, etc. 
    a. Note the data structure and data types.
    a. Note the dimension of your dataset, the number of rows and number of columns</p>
1. Look at the data summary (`summary()`) and identify if there are any missing data or anything else that looks unusual for some of the variables.</p>
1. Use some of the functions that we have used below to tidy your data
    a. Do you need to simplify column names?
    a. Can you remove some of the columns to form a small dataframe?
    a. Do you need to created a binary variable coding if some variables are usable or not? 
    a. Do you need to create some new variables (`mutate()`)?
    a. Do you need to filter out some of the data (`filter()`)?</p>
1. Now you can do some summaries using the `group_by` function if you have grouping variables, and the `summarise` function.</p>
1. Take time to explore a variety of plot types, even if you wouldn't necessarily use them all, but just to get more experience with the `ggplot2` syntax and the breath of available types of plots. </p>
1. Ask questions! We're here to help, and if there is something that we have not covered in the workshop material but that you would like to learn how to do, we will be happy to help you out!</p>
